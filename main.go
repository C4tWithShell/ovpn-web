...
func main() {
	...
	r.POST("/user", func(c *gin.Context) {
		...
		if err != nil {
			...
			c.JSON(http.StatusOK, gin.H{"message": "User is added successfully"})
		}
	})
	...
	ovpn := r.Group("/ovpn")
	{
		...
		ovpn.POST("/server", func(c *gin.Context) {
			...
			case "renewcert":
				...
				ov.sendCommand("signal SIGHUP")
				c.JSON(http.StatusOK, gin.H{"message": "Certificate update successful"})
			case "restartSrv":
				_, err := ov.sendCommand("signal SIGHUP")
				if err != nil {
					logger.Error(context.Background(), err.Error())
					c.JSON(http.StatusInternalServerError, gin.H{"message": "Restart service failed"})
					return
				}

				c.JSON(http.StatusOK, gin.H{"message": "Restart service successfully"})
			case "getConfig":
			...
			case "updateConfig":
				...
				c.JSON(http.StatusOK, gin.H{"message": "Configuration update successful"})
			default:
		...
		ovpn.PUT("/client", func(c *gin.Context) {
			...
			if strings.Contains(f, "ccd") {
				...
				if err != nil {
					...
					msg += "（CCD is not enabled and the service needs to be restarted）"
				}
			}
		...
		ovpn.POST("/client", func(c *gin.Context) {
			...
			if err != nil {
				...
				if out, err := cmd.CombinedOutput(); err != nil {
					...
					logger.Error(context.Background(), string(out))
					c.JSON(http.StatusInternalServerError, gin.H{"message": "Failed to add client"})
					return
				}
			} else {
				c.JSON(http.StatusUnprocessableEntity, gin.H{"message": "Client already exists"})
				return
			}
		...
		ovpn.DELETE("/client/:name", func(c *gin.Context) {
			...
			if out, err := cmd.CombinedOutput(); err != nil {
				...
				logger.Error(context.Background(), string(out))
				c.JSON(http.StatusInternalServerError, gin.H{"message": "Failed to delete client"})
				return
			}
			...
			c.JSON(http.StatusOK, gin.H{"message": "Deleting the client successfully"})
		})
		...
		ovpn.POST("/history", func(c *gin.Context) {
			...
			if err != nil {
				...
				c.JSON(http.StatusOK, gin.H{"message": "Add record successfully"})
			}
		})
	}
